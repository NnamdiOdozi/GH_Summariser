...
- `user_prompt` — Short instruction to focus the summary on a specific area (e.g., `"Focus on the authentication module"`)
...
By default, the LLM receives a general-purpose summarization prompt. You can steer the summary towards a specific area by providing a `user_prompt`. Example prompts:
